...
public class MiddlewareFigure extends AbstractTextControlContainerFigure {

	 public MiddlewareFigure() {
		...
		protected void drawIconImage(Graphics graphics) {
			...
			ImageData data = new ImageData(path + "/" + IArchiImages.ICON_MIDDLEWARE_IMAGE);
			Image image = new Image(display, data);

			int w = image.getImageData().width;
			int h = image.getImageData().height;
			graphics.drawImage(image, 0, 0, w, h, pt.x, pt.y, 19, 19);
			graphics.popState();
			image.dispose();
		}
